...
class PixelCNN(nn.Module):
  ...
  def forward(self, x):
    ...
    R_final = torch.cat((R_aux1, R_aux2, R), 1) # [128, 3, 32, 32]
    ...
    G_final = torch.cat((G_aux, R, G), 1) # [128, 3, 32, 32]
    ...
    B_final = torch.cat((R, G, B), 1) # [128, 3, 32, 32]

    for idx in range(len(self.conv_layers)):

      B_final = self.activation(self.batch_norms[idx](self.conv_layers[idx](B_final)))

    return self.conv_out(R_final), self.conv_out(G_final), self.conv_out(B_final)
